...
import com.example.xpuzzle.PuzzleMain;
import com.example.xpuzzle.R;
...
public class ImagesUtil {
        ...
        for (int i = 1; i <= type; i++){
            for (int j = 1; j <= type; j++){
                bitmap = Bitmap.createBitmap(picSelected,
                        ...
                        itemWidth,
                        itemHeight);
                bitmapItems.add(bitmap);//放入集合
                itemBean = new ItemBean(//设置每个小图片的Item基本属性
                        (i-1)*type + j,
                        (i-1)*type + j,
                        bitmap);
                GameUtil.mItemBeans.add(itemBean);//给GameUtil.mItemBeans集合中添加，每个切割好的小图片
            }
        }
        // 保存最后一个图片在拼图完成时填充
    ...
    public Bitmap resizeBitmap(float newWidth, float newHeight, Bitmap bitmap){
        ...
        if (bitmap != null){
            ...
            newBitmap = Bitmap.createBitmap(bitmap, 0, 0, (int)newWidth, (int)newHeight, matrix, true);
        }
//        matrix.postScale(newWidth / bitmap.getWidth(), newHeight / bitmap.getHeight());//缩放大小
